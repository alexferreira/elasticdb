...
PromiseB.promisifyAll(elasticsearch);
 ...
 * @param callback
 */
ElasticDb.prototype.connect = function(options) {
  var self = getInstance();
...
ElasticDb.prototype.index = function(obj, schema, options) {
  ...
  return this.elasticClient.index({
    ...
    Logger.info('save operation succeed', resp);
  }).catch(function(err){
    Logger.error('save operation failed', err);
  });
};
...
ElasticDb.prototype.remove = function(id, options) {
  return this.elasticClient.delete({
    ...
    Logger.info('delete operation succeed', resp);
  }).catch(function(err){
...
ElasticDb.prototype.syncModel = function(modelName) {
  ...
  return new PromiseB(function(resolve, reject) {
    stream.on('data', function (obj) {
      ...
      self.index(obj, schema, options).then(resume);
...
ElasticDb.prototype.search = function(options, query) {
  ...
  options = _.omit(options, 'logLevel', 'host', 'apiVersion');

  return new PromiseB(function(resolve, reject) {
    self.elasticClient.search(options, function(err, resp) {
      if(err) reject(err);
